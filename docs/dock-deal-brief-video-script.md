...
- **Hosting:** YouTube (unlisted is fine). Once produced, drop the video id into
  `src/data/scenarioBriefs.ts` → `"apex-positioning"` → `briefVideo.youtubeId`
  (replace the `"PENDING"` sentinel). Nothing else needs to change.
