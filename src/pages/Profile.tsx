...
function StatCard({ icon: Icon, label, value }: { icon: React.ElementType; label: string; value: string }) {
    return (
          <div className="bg-white/5 rounded-xl p-4 border border-white/10 flex items-start gap-3">
                ...
                <div>
                        <p className="text-white/50 text-xs">{label}</p>p>
                        <p className="text-white font-medium text-sm mt-0.5">{value}</p>p>
                </div>div>
          </div>div>
        );
}
    ...
    if (loading) {
          return (
                  <div className="min-h-screen bg-[#0a1628] flex items-center justify-center">
                          <div className="text-cyan-400 animate-pulse">Loading your profile…</div>div>
                  </div>div>
                );
    }
                ...
                <div className="max-w-2xl mx-auto space-y-6">
                  ...
                  {/* ── Hero card ── */}
                        <div className="bg-white/5 rounded-2xl p-8 border border-white/10 text-center">
                                  ...
                                  <h1 className="text-3xl font-bold text-white">{profile?.full_name || 'Apex Member'}</h1>h1>
                                  ...
                                  <div className="space-y-3">
                                              <div className="flex items-center justify-between p-3 bg-white/5 rounded-xl">
                                                            <div className="flex items-center gap-3">
                                                                            ...
                                                                            <div>
                                                                                              <p className="text-white text-sm font-medium">Pre-Program Survey</p>p>
                                                                                              <p className="text-white/40 text-xs">Demographics, knowledge, confidence &amp; intentions</p>p>
                                                                            </div>div>
                                                            </div>div>
                                                {preSurvey ? (
                            <span className="text-green-400 text-xs font-medium">Done</span>span>
                          ) : (
                            <Link to="/onboarding" className="text-cyan-400 text-xs font-medium flex items-center gap-1">
                                              ...
                                              <div className="flex items-center justify-between p-3 bg-white/5 rounded-xl">
                                                            <div className="flex items-center gap-3">
                                                                            ...
                                                                            <div>
                                                                                              <p className="text-white text-sm font-medium">Post-Program Survey</p>p>
                                                                                              <p className="text-white/40 text-xs">Reaction, learning gains, behavior &amp; outcomes</p>p>
                                                                            </div>div>
                                                            </div>div>
                                                {postSurvey ? (
                            <span className="text-green-400 text-xs font-medium">Done</span>span>
                          ) : (
                            <span className="text-white/30 text-xs">Unlocks at program end</span>span>
                                                            )}
                                              </div>div>
                                  </div>div>
                        </div>div>
