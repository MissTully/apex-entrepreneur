...
export default function Certificate() {
  ...
  if (loading) return <div className="min-h-screen bg-[#0a1628] flex items-center justify-center"><div className="text-cyan-400 animate-pulse">Loading certificate...</div>div></div>div>;
  
    if (locked) return (
          <div className="min-h-screen bg-[#0a1628] flex items-center justify-center px-4">
                <div className="text-center max-w-md">
                        <Award className="w-16 h-16 text-white/20 mx-auto mb-4" />
                        <h1 className="text-2xl font-bold text-white mb-3">Certificate Locked</h1>h1>
                        <p className="text-white/60 mb-6">Complete the post-program survey to unlock your Certificate of Completion from the Hillsborough County Entrepreneurship Center.</p>p>
                        <Link to="/post-survey" className="inline-flex items-center gap-2 bg-cyan-500 hover:bg-cyan-400 text-white font-semibold px-6 py-3 rounded-xl">Take Post-Program Survey</Link>Link>
                </div>div>
          </div>div>
        );
          ...
          <div className="min-h-screen bg-[#0a1628] px-4 py-12">
                <div className="max-w-2xl mx-auto">
                        ...
                        <div className="relative bg-gradient-to-br from-[#0d1e3a] via-[#0a1628] to-[#071020] rounded-3xl border-2 border-yellow-400/40 p-10 text-center overflow-hidden">
                                  ...
                                  <p className="text-yellow-400/70 text-xs font-semibold tracking-widest uppercase mb-6">Hillsborough County Entrepreneurship Center</p>p>
                                  ...
                                  <p className="text-white/60 text-sm mb-2 tracking-wide uppercase">Certificate of Completion</p>p>
                                  <p className="text-white/50 text-sm mb-6">This is to certify that</p>p>
                                  <h1 className="text-4xl font-bold text-white mb-2">{cert!.name}</h1>h1>
                                  <p className="text-white/40 text-sm mb-8">{cert!.email}</p>p>
                                  <p className="text-white/70 text-base leading-relaxed max-w-md mx-auto mb-8">
                                              has successfully completed all five phases of the <span className="text-cyan-300 font-semibold">Apex Entrepreneur Program</span>span> including live experiential sessions, AI-powered simulations, and pre- and post-program assessments, demonstrating mastery of entrepreneurial leadership, communication, and venture development.
                                  </p>p>
                          ...
                          <div key={p} className="flex items-center gap-2 text-white/60 text-xs">
                                  ...
                                  <div className="border-t border-white/10 pt-6 flex flex-col sm:flex-row items-center justify-between gap-4">
                                              <div><p className="text-white/30 text-xs">Date of Completion</p>p><p className="text-white/70 text-sm font-medium">{formatDate(cert!.completedAt)}</p>p></div>div>
                                              <div><p className="text-white/30 text-xs">Certificate ID</p>p><p className="text-cyan-400 text-sm font-mono font-medium">{cert!.certId}</p>p></div>div>
                                              <div><p className="text-white/30 text-xs">Issued by</p>p><p className="text-white/70 text-sm font-medium">HCEC - Apex Program</p>p></div>div>
                                  </div>div>
                        </div>div>
                        ...
                        <p className="text-center text-white/30 text-xs mt-4">
                                  Certificate ID <span className="text-cyan-400 font-mono">{cert!.certId}</span>span> can be verified by the Hillsborough County Entrepreneurship Center.
                        </p>p>
                </div>div>
          </div>div>
        );
